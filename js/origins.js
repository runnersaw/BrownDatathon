...
function createOriginsMap(d) {
    data = {}
	for (var country in d) {
        var val = d[country];
        var risk = 'defaultFill';
        if (mode == 'all') {
            if (val < 10) {
                risk = 'defaultFill';
            } else if (val < 50) {
	...
	map = new Datamap({
		element: document.getElementById('origins-container'),
		projection: 'mercator',
		responsive: true,
		scope: "world",
		...
		geographyConfig: {
			...
			popupTemplate: function(geography, data) {
				return '<div class="hoverinfo">'+data.amount+' attacks originated from '+geography.properties.name+' '
			},
...
function load_origins() {
	$.getJSON("./js/origins.json", function(json) {
		$('#origins-container').empty();
        if (mode == 'all') {
            createOriginsMap(json['all']);
        } else {
            createOriginsMap(json[year.toString()]);
        }
	});
}
...
d3.select('#slider').call(d3.slider().axis(true).min(1995).max(2015).step(1).on('slide', function(evt, value) {
  year = value;
  if (mode == 'year') {
    load_origins();
  }
...
function updateForMode() {
  if (mode == 'all') {
    ...
    $('#years-button').html("View individual years");
  } else {
    $('#slider').css({'opacity': 1.0});
    $('#years-button').html("View all years");
  }
}
